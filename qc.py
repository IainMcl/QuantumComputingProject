...
class Operator():
    ...
    def __mod__(self, other):
        ...
        if not isinstance(other,Operator):
            raise TypeError("Cannot tensor product {} with an operator".format(other.type))
        n_qubits = self.n_qubits + other.n_qubits
        result = Operator(n_qubits)
...
class Unitary(Operator):
    ...
    def __init__(self, theta, phi, n_qubits=1):
        self.base = np.array([[np.cos(theta), np.sin(theta) * -1j * np.exp(-1j * phi)],
                              [np.sin(theta) * -1j * np.exp(-1j * phi), np.cos(theta)]])
